...
def CJspeed(P1, T1, q, mech, plt_num):
    ...
    WR: Uses the minimum wave speed algorithm
...
def shk_calc(U1, gas, gas1, ERRFT, ERRFV):
    ...
    while(abs(deltaT) > ERRFT*T or abs(deltaV) > ERRFV*V):
        ...
        #TEMPERATURE PERTURBATION (2% of current temperature), keep volume constant
        DT = T*0.02; Tper = T + DT;
        Vper = V; Rper = 1/Vper;
        ...
        #VOLUME PERTURBATION (2% of current volume), keep temperature constant
        DV = 0.02*V; Vper = V + DV;
        Tper = T; Rper = 1/Vper;
        [Pper, Hper] = state(gas,Rper,Tper)
        #CALCULATE FHX & FPX FOR "IO" STATE
        ...
        #CHECK & LIMIT CHANGE VALUES
        #TEMPERATURE
        DTM = 0.2*T		# This was chosen to accelerate convergence in temperature
        if abs(deltaT) > DTM:
